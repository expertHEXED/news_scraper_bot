...
def get_currency():
    try:
        ...
        # Browser kabi ko'rinish uchun header
        headers = {"User-Agent": "Mozilla/5.0"}
        res = requests.get(url, headers=headers, timeout=10).json()
        
        # Dollar va Evroni qidirish (Ccy va Rate kalitlari bilan)
        usd = next(x for x in res if x.get("Ccy") == "USD")
        eur = next(x for x in res if x.get("Ccy") == "EUR")
        
        return (f"💰 **Rasmiy valyuta kursi:**\n\n"
                f"🇺🇸 1 USD = {usd['Rate']} so'm\n"
                f"🇪🇺 1 EUR = {eur['Rate']} so'm")
    except Exception as e:
        return "⚠️ Kursni olishda texnik xatolik yuz berdi."
...
def start(message):
    ...
    bot.send_message(
        message.chat.id, 
        f"Salom {message.from_user.first_name}! Bot qayta tiklandi.\nKerakli bo'limni tanlang:", 
        reply_markup=m
    )
...
def handle(msg):
    if msg.text == "💹 Valyuta kursi":
        ...
        bot.send_message(msg.chat.id, get_news(), parse_mode="Markdown")
